...
- reverse (boolean): De volgorde van de jaren wordt omgedraaid en er wordt gekeken naar hoe lang het gewenste LU aanwezig is. 

**Voorbeeld**

```{r mask_follow_up, eval = FALSE, include = TRUE, echo = TRUE}
mask_follow_up(c("1123", "2235", "3333"), clustering_position = 2)

mask_follow_up(c("1123", "2235", "3333"), clustering_position = 1, reverse = TRUE)
```
...
De functie past_follow_up clustert een reeks landgebruikscodes (LU-codes) op basis van een opgegeven clusteringpositie. Dit betekent dat alle voorgaande karakters voor de clusteringpositie worden gecontroleerd en vervangen door "X" zodra een verandering in landgebruik wordt gedetecteerd.
...
```{r past_follow_up, eval = FALSE, include = TRUE, echo = TRUE, warning = TRUE}

past_follow_up(c("2123", "4323", "1333", "1355"),
  position  = 4,
  LU = "3"
)

past_follow_up(c("1223", "2325", "3233", "5525"),
  position = 2,
  LU = "2"
)
```
...
De functie mask_follow_up_intensity bouwt verder op mask_follow_up, maar voegt extra logica toe om de intensiteit van het landgebruik te bepalen. Dit wordt gedaan door te kijken of het landgebruik als intensief (I) of extensief (E) wordt geclassificeerd.
...
mask_follow_up_intensity(c("11123", "22345", "33333"), 
                         LU = 2, clustering_position = 2)
```
output:

```{r echo = FALSE}
mask_follow_up_intensity(c("11123", "22345", "33333"), LU = 2,
...
Hier wordt gekeken of het 1ste karakter (clustering) en het 3de karakter (referentie) gelijk zijn.
...
De clustering werd toegepast op de gegeven dataset. De functie 'change_frequency' werd gebruikt als clusteringsmethode. Na de clustering werd met de functie calculate_accuracy de overall accuracy (OA) de user accuracy (UA) en producer accuracy bepaald (PA) (@tbl-output_clustering) en geplot (@fig-UA_PA). Tijdens deze berekening werd er geen rekening gehouden met de oppervlakte verhouding van elk stratum. 
...
OA_LL <- round(100 * unname(res1$overall["AccuracyLower"]), digits = 2)
OA_UL <- round(100 * unname(res1$overall["AccuracyUpper"]), digits = 2)
...
#| tbl-cap: !expr 'paste("De totale accuraatheid is ", OA , " met een 95% CI van [", OA_LL, ";", OA_UL, "]." )'
...
```{r plot_UA_PA, eval = TRUE}
#| label: fig-UA_PA
#| fig-cap: "Plot of the PA and UA after clustering."
...
saveRDS(test_clustering, "./data/output_clustering.RDS")
saveRDS(data_LU_area_clus, "./data/areas_LU_clustered.RDS")
