...
data <- data %>%
  ...
  rename(
    reference = LU_1969_1873_1774_true,
    observed = LU_2022_1969_1873_1774_pred
  ) %>%
  mutate(
    reference_comp = as.integer(paste0(
      year_1969,
      year_1873,
      year_1774
    )),
    reference_comp4 = as.integer(paste0(
      year_2022,
      year_1969,
      year_1873,
      year_1774
...
data <- data %>%
  mutate(
    observed3 = ifelse(nchar(observed) == 3,
      observed,
...
```{r mask_follow_up, eval = FALSE, include = TRUE}
mask_follow_up(c("11123", "22345", "33333"), clustering_position = 2, reverse = FALSE)

mask_follow_up(c("11123", "22345", "33333"), clustering_position = 2, reverse = TRUE)
```
output:
...
De functie past_follow_up clustert een reeks landgebruikscodes (LU-codes) op basis van een opgegeven clusteringpositie. Dit betekent dat alle voorgaande karakters voor de clusteringpositie worden gecontroleerd en vervangen door "X" zodra een verschil wordt gedetecteerd.

**Parameters**

- variable (character vector): Een vector met landgebruikscodes, elk bestaande uit een reeks karakters.
- clustering_position (integer): De index van het karakter waarop clustering wordt gebaseerd. Dit is de startpositie van de clustering.
- LU (character): De landgebruikscode waarnaar er gezocht wordt op de clusteringspositie. 
- years (numeric vector): Een vector met de geobserveerde jaren (standaard c(2022, 1969, 1879, 1774))

 Waarschuwingen worden gegeven wanneer:
 - er een andere codering voor de LU gegeven is en
 - wanneer het LU niet voorkomt in het opgegeven jaar. 
...
```{r past_follow_up, eval = FALSE, include = TRUE}
past_follow_up(c("21232", "45233", "13333"),
  position  = 4,
  LU = "3"
)

past_follow_up(c("11123", "22345", "33333"),
  position = 2,
  LU = "2"
)
```
...
```{r echo = FALSE}
past_follow_up(c("21232", "45233", "13333"),
  clustering_position = 4,
  LU = "3"
)

past_follow_up(c("11123", "22345", "33333"),
  clustering_position = 2,
  LU = "2"
)
```
...
make_bin_change(c("12345", "22345", "33345"), clustering_position = 1, reference_position = 3)
```
Output:
...
Input:
```{r echo = TRUE, eval = FALSE}
change_frequency(c("12345", "22345", "33445"),
...
Input:
```{r echo = TRUE, eval = FALSE}
presence(c("12345", "22345", "33445", "41256", "44156"),
  clustering_position = 1,
...
Input:
```{r echo = TRUE, eval = FALSE}
transition_present(c("12345", "22345", "33445"),
  clustering_position = 1,
...
clustering_year = 1873)

res1 <- calculate_accuracy(
  mapdata = test_clustering$prediction_change_frequency,
...
data_LU_area_clus <- data_LU_merge %>%
  group_by(reference_change_frequency) %>%
  summarise(
    area_ha = sum(ha, na.rm = TRUE),
...
# using MPAC package
aa <- mapac::aa_card(
  data = test_clustering[, c(
    "reference_change_frequency",
...
maparea_RD <- data_LU_area_clus %>%
  rownames_to_column(var = "map") |>
